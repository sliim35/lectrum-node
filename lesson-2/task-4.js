class TimersManager {
  ...
  start() {
      ...
      const timer = setTimeout(
        function run() {
          ...
          try {
            ...
            if (item.interval) {
              setTimeout(run.bind(this), item.delay);
            }
            this._log({
              name: item.name,
              in: item.params,
    ...
    // Clear all timers after +10 sec
    const longestTimerTime = this.timers.reduce((acc, { delay }) => {
      if (delay > acc) return delay;
      return acc;
    }, null);
    console.log('longestTimerTime', longestTimerTime);
    setTimeout(
      () => this.timers.forEach(({ timer }) => clearTimeout(timer)),
      longestTimerTime + 1000
    );
  }
